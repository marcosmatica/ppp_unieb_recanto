...
export default function SchoolsPage() {
  ...
  useEffect(() => {
    ...
    schoolsService.getByCRE(profile.cre).then(data => {
      setSchools(data)
      setLoading(false)
    })
  }, [profile])
  ...
  function openEdit(school) {
    setForm({
      ...
      stages: {
        ...Object.fromEntries(STAGES_CONFIG.map(s => [s.key, false])),
        ...school.stages,
      },
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">
            {schools.length} unidade{schools.length !== 1 ? 's' : ''} cadastrada{schools.length !== 1 ? 's' : ''} · {profile?.cre}
          </p>
        </div>
        ...
        {schools.map(school => {
          ...
          return (
            <div key={school.id} className="school-card animate-fade-in">
              <div
                className="school-card-main"
                onClick={() => setExpandedId(expanded ? null : school.id)}
              >
                ...
                <div className="school-card-actions">
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); openEdit(school) }}
                  >
                    Editar
                  </button>
              ...
              {expanded && (
                <div className="school-card-detail animate-fade-in">
                  ...
                  <p className="detail-hint">
                    ...
                    {STAGES_CONFIG
                      .filter(s => school.stages?.[s.key] && s.note)
                      .map(s => s.note)
                      .join(' · ') || 'configuração padrão'}
                  </p>
                </div>
              )}
            </div>

...
export default function ReportPage() {
  ...
  async function handleGenerate() {
    if (pending > 0)  return toast.error(`${pending} elemento(s) ainda não revisados`)
    if (!decision)    return toast.error('Selecione a decisão final')
    setGenerating(true)
    try {
      ...
      a.download = `parecer_${analysis.schoolName.replace(/\s+/g,'_')}_${analysis.year}.docx`
      a.click()
    ...
    <div className="page">
      ...
      <div className="report-grid">
        ...
        <div className="report-summary-col">
                ...
                {list.map(el => (
                  <div key={el.elementId} className="rgroup-item">
                    ...
                    {el.aiResult?.legalRefs?.missing?.length > 0 && (
                      <div className="rgi-legal">
                        Refs. ausentes: {el.aiResult.legalRefs.missing.join(', ')}
                      </div>
                    )}
                  </div>
                ))}
          ...
          {adequateList.length > 0 && (
            <div className="report-group group-adequate">
              <div className="rgroup-header"><CheckCircle2 size={14}/><span>Adequados ({adequateList.length})</span></div>
              {adequateList.map(el => (
                <div key={el.elementId} className="rgroup-item compact">
                  <div className="rgi-label">{el.label}</div>
                </div>
              ))}
            </div>
